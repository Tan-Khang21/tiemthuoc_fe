<template>
  <el-card>
    <div class="top-row">
      <div class="filters-row" role="region" aria-label="Bộ lọc hóa đơn online">
        <el-date-picker
          v-model="startDate"
          type="date"
          class="filter-date"
...
const applyFilters = () => {
  ...
  const params = { from: formatDateOnly(startDate.value), to: formatDateOnly(endDate.value) };
  ...
  api.hoadon.search(params)
    ...
    .catch(err => {
      console.error('search hoadon online error', err);
...
const resetFilters = () => {
  startDate.value = null; endDate.value = null; status.value = null; type.value = 'HDOL'; searchKeyword.value = '';
};
  ...
  ElMessageBox.confirm(`Bạn có chắc muốn hủy hóa đơn "${id}"?`, 'Xác nhận', { confirmButtonText: 'Hủy', cancelButtonText: 'Bỏ qua', type: 'warning' })
    .then(() => {
      ...
      api.hoadon.cancel(id)
        .then(resp => {
          const r = (resp && resp.data) ? resp.data : resp;
          if (r && r.status === -1) { ElMessage.error(r.message || 'Hủy thất bại'); return; }
          ElMessage.success('Hủy hóa đơn thành công');
          applyFilters();
        })
        .catch(err => { console.error('cancel hoadon err', err); ElMessage.error('Hủy thất bại'); })
        .finally(() => { loading.value = false; });
    }).catch(() => {});
};
...
const exportExcel = () => {
  try {
    ...
    const fileName = `HoaDonOnline_${startDate.value || 'from'}_${endDate.value || 'to'}.csv`;
  ...
  } catch (err) { console.error('export csv err', err); ElMessage.error('Xuất Excel thất bại'); }
};
...
const printList = async () => {
  ...
  if (currentUser?.MaNV) {
    try { const resp = await api.nhanvien.getById(currentUser.MaNV); const nv = resp && resp.data ? resp.data : resp; tenNhanVien = nv?.hoTen || nv?.tenNV || nv?.TenNV || nv?.hoVaTen || currentUser.TenDangNhap || ''; } catch (err) { console.error('get nhanvien error', err); }
  }
...
function debounce(fn, wait=400){ let t=null; return (...args)=>{ if(t) clearTimeout(t); t=setTimeout(()=>fn(...args), wait); }; }
const debouncedApply = debounce(()=>applyFilters(), 450);
watch([startDate, endDate, status], ()=>{ debouncedApply(); });
watch(searchKeyword, ()=>{ debouncedApply(); });
</script>

<style scoped>
/* reuse same styles as hoadon.vue filters/table */
.filters-row { display:flex; gap:6px; flex-wrap:nowrap; align-items:center; overflow-x:auto; max-width:100%; }
.filter-actions { display:flex; gap:8px; align-items:center; }
.filters-row > * { flex: 0 0 auto; }
.filter-date { min-width:120px; max-width:180px; }
.filter-date .el-input__inner { height:44px; padding:6px 10px; border-radius:6px; font-size:13px; box-shadow:none; }
.filter-date .el-input__inner::placeholder { color:#9aa3ad; }
.filter-small { min-width:100px; max-width:160px; }
.filter-small .el-input__inner, .filter-search .el-input__inner { height:42px; padding:6px 10px; border-radius:6px; font-size:13px; }
